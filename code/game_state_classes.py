...
class Game_state:
    def __init__(self):
        self.we_play_white = (
            True  # This store the player color, it will be changed later
        )
        self.moves_to_detect_before_use_engine = (
            -1
        )  # The program uses the engine to play move every time that this variable is 0
        ...
        self.engine = chess.uci.popen_engine(
            engine_path
        )  # The engine used is stockfish. It requires to have the command stockfish working on the shell
    ...
    def can_image_correspond_to_chessboard(self, move, current_chessboard_image):
        ...
        for square in squares:
            ...
            squareImage = get_square_image(
                rowOnImage, columnOnImage, current_chessboard_image
            )
    ...
    def get_valid_move(
        self, potential_starts, potential_arrivals, current_chessboard_image
    ):
        ...
        for start in potential_starts:
            for arrival in potential_arrivals:
                ...
                if move in self.board.legal_moves:
                    if self.can_image_correspond_to_chessboard(
                        move, current_chessboard_image
                    ):  # We only keep the move if the current image looks like this move happenned
        ...
        if (
            ("e1" in potential_starts)
            and ("h1" in potential_starts)
            and ("f1" in potential_arrivals)
            and ("g1" in potential_arrivals)
        ):
        ...
        if (
            ("e1" in potential_starts)
            and ("a1" in potential_starts)
            and ("c1" in potential_arrivals)
            and ("d1" in potential_arrivals)
        ):
            valid_move_string = "e1c1"

        # Detect castling king side with black
        if (
            ("e8" in potential_starts)
            and ("h8" in potential_starts)
            and ("f8" in potential_arrivals)
            and ("g8" in potential_arrivals)
        ):
        ...
        if (
            ("e8" in potential_starts)
            and ("a8" in potential_starts)
            and ("c8" in potential_arrivals)
            and ("d8" in potential_arrivals)
        ):
    ...
    def register_move_if_needed(self):
        ...
        valid_move_string1 = self.get_valid_move(
    ...
    def get_square_center(self, square_name):
        ...
        centerX = int(
            position.minX + (column + 0.5) * (position.maxX - position.minX) / 8
        )
    ...
    def play_next_move(self):
        ...
        if best_move.promotion != None:
            ...
            cv2.waitKey(100)
            pyautogui.dragTo(
                centerXDest, centerYDest + 1, button="left", duration=0.1
            )  # Always promoting to a queen
