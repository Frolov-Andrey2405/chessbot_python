...
def find_chessboard():
    ...
    is_found, current_chessboard_image, minX, minY, maxX, maxY, test_image = (
        find_chessboard_from_image(img)
    )
...
def get_chessboard(game_state):
    ...
    monitor = {
        "top": 0,
        "left": 0,
        "width": position.maxX + 10,
        "height": position.maxY + 10,
    }
...
def find_chessboard_from_image(img):
    ...
    if found_board:
        ...
        resizedChessBoard = cv2.resize(
            image[columnStart:columnEnd, lineStart:lineEnd],
            dim,
            interpolation=cv2.INTER_AREA,
        )
        return (
...
def test_chessboard_detection(imageDir, expectedBoard):
    ...
    for file in os.listdir(imageDir):
        ...
        found_board, resizedChessBoard, minX, minY, maxX, maxY, info_image = (
            find_chessboard_from_image(image)
        )
...
def global_test_chessboard_detection():
    ...
    image_count, error_count = test_chessboard_detection(
        "TestImages/withoutBoard/", False
    )
    print(
        "Errors with images not having a board : ", 100 * error_count / image_count, "%"
    )
