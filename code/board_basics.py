...
def get_square_image(
    row, column, board_img
):  # this functions assumes that there are 8*8 squares in the image, and that it is grayscale
...
def convert_square_name_to_row_column(
    square_name, is_white_on_bottom
):  # Could be optimized
    # print("Looking for " + repr(square_name))
    for row in range(8):
        for column in range(8):
            this_square_name = convert_row_column_to_square_name(
...
def get_square_center_from_image_and_move(
    square_name, is_white_on_bottom, minX, minY, maxX, maxY
):
...
def has_square_image_changed(
    old_square, new_square
):  # If there has been a change -> the image difference will be non null -> the average intensity will be > treshold
...
def is_square_empty(square):  # A square is empty if its pixels have no variations
    return (
        square.std() < 10
    )  # 10 works pretty well -> the mouse pointer is not enought to disturb (but sometimes it actually does, especially with small chessboards and big pointer)
...
def is_white_on_bottom(current_chessboard_image):
    ...
    m2 = get_square_image(
        7, 7, current_chessboard_image
    ).mean()  # Rook on the bottom right
    ...
    for row in range(8):
        ...
        for column in range(8):
            ...
            if has_square_image_changed(old_square, new_square):
                square_name = convert_row_column_to_square_name(
                    row, column, is_white_on_bottom
                )
